...
3. Read the response's `result` and relay it to the caller verbatim, refusal or not. Done when the caller has the complete result and no further action has been taken on the reviewed bundle; not done if the result was summarized, softened, or followed by any change to the reviewed bundle.
