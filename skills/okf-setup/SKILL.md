...
```json
{
  ...
  "payload": {
    ...
    "task_kind": "feature work",
    "staged": [
      { "path": "docs/decisions/use-postgres.md", "concept": "decisions/use-postgres", "type": "Decision", "shard": "docs/decisions", "file": ".okf-staging/okf/decisions/use-postgres.md" }
    ]
  }
}
```

`publish` (#149) is the operation that turns a staged bundle `migration-validate` reported `data.publishable: true` for into real bundle concepts, and it is the only operation in this skill that ever reaches the real bundle. `payload.staged` is exactly `assemble`'s own `data.staged` array, unmodified — the same "reuse the upstream operation's own shape" rule `assemble` and `migration-validate` already follow for their own upstream payloads. `payload.task_kind` must be one of the write-eligible task kinds (`feature work`, `fix`, `research`) the write gate already recognizes; anything else is `TASK_KIND_NOT_WRITE_ELIGIBLE` before anything is dispatched.
...
6. **Repair `.okf-workspace.json` through `repair`, when approved.** Read `inspect`'s report for this file first:
   ...
   - A `monorepo: true` hint is a warning, not a decision: call `plan` before asking the user anything. Done when step 13 has run and either produced a deterministic package layout or reported the layout as a question; not done if a single-bundle or hand-written manifest is chosen while `plan` has not yet been tried.
...
8. **Derive the migration plan and run the one compact batched question round (#144).** Call `migration-plan` with `payload.sources` set to step 7's `data.sources`, unmodified. Done when a response is read with `data.plan` and `data.questions`; not done while any entry's disposition is assumed rather than read from `data.plan.entries`.
   ...
   - A determined plan's entry-level vocabulary (`migrate`/`skip`/`residue`/`blocked_pending_decision`, an intent) is deliberately distinct from `report`'s source-level vocabulary in step 17 (`migrated`/`skipped`/`ambiguous`/`residue`, an outcome): `data.mapping` already carries each `migrate` entry's extracted provenance and link-rewritten body (#145); step 9 partitions that same plan into the shards fresh-context workers actually convert, step 10 assembles what those workers actually returned, step 11 validates what was assembled, step 12 publishes what step 11 declared publishable, and step 17's payload is always built from what that work actually did, never copied straight from this step's plan.
...
11. **Validate the staged bundle before publication (#148).** Once step 10's `assemble` call has run, call `migration-validate` with `payload.selected` set to step 7's own `data.sources` paths, `payload.plan` set to step 8's own `data.plan`, unmodified, and `payload.semantic_review` — honestly, not yet claiming a review that has not happened, since that comes only at step 17. Done when `data.status` and `data.publishable` are both read; not done while a structural finding, a `SOURCE_DISPOSITION_MISSING` finding, or a `data.publishable: false` is treated as anything other than blocking the next step.
    ...
    - `data.publishable: true` with no blocking finding is this procedure's own signal that step 12's publication may proceed; it never proceeds while `data.publishable: false`.
12. **Publish the validated staged bundle (#149).** Once step 11 reports `data.publishable: true`, call `publish` with `payload.staged` set to step 10's own `data.staged`, unmodified, and a write-eligible `payload.task_kind`. Done when `data.status`, `data.published`, and `data.failed` are all read; not done while `data.publishable: false` from step 11 is worked around, and not done while a concept in `data.failed` is treated as if it had been created.
    - `data.status: "complete"` — every staged concept's own delegation receipt was `"clean"`; the bundle now carries every concept step 10 staged.
    - `data.status: "partial"` — `data.failed` names each concept that did not publish and its own delegation `status`; a `CONCEPT_ALREADY_EXISTS` finding on one names a target collision with something already in the bundle (a decision for the user, same as step 10's own `CONCEPT_TARGET_COLLISION`, never resolved by retrying); any other finding names a write-gate refusal to correct before retrying just that concept. This is the operation this whole procedure exists to reach honestly: report exactly which concepts are real and which are not, never a blanket "migration complete" while `data.failed` is non-empty.
    - `result: "blocked"`, `data.code: "PUBLISH_PRECHECK_FAILED"` — the target bundle itself is not currently viable to write into (for example, `index.md` no longer declares `okf_version: "0.2"`); nothing was attempted. Repair the bundle root (step 4) and retry.
13. **For a monorepo, detect package boundaries and dispatch one sub-agent per package, in parallel (#135).** Call `plan` with the bundle name and, when already known, `project_mode` and approved `mappings`. Done when the result is one of the three states below and handled accordingly; not done while any state is treated as another.
    - `data.monorepo: false` — proceed as a single-project setup (steps 1–12 as written); this repository has at most one package.
