...
- `SPLIT_SECTION_SPANS_HEADING` (`line_start`, `line_end`, `heading_line`) — one range swallows a further heading. A heading and its content are one section, so several headings reaching one output are listed as several sections sharing an `output`, never merged into one range.
...
- `SPLIT_SOURCE_CHANGED` (`expected`, `actual`) — `source_identity` was supplied and no longer matches the source on disk. #200: a source change invalidates the proposal before transformation or publication.
...
`payload.answers`, when supplied, is a plain object keyed by source path. Every key must still name a question this same `payload.sources` produces and every value must be one of that question's own closed `options` (or, for `"type"`, a non-empty string) — an answer naming a question that is not open, or a value outside the question's own options, is `UNSUPPORTED_INPUT` before anything is computed, never guessed past or silently ignored. Answering only some of the open questions resolves exactly those and leaves the rest open; there is no requirement to answer everything in one call, only no ability to treat the plan as executable until every question is. `payload.split_requested`, when supplied, must be an array of non-empty strings — anything else is `UNSUPPORTED_INPUT` before anything is computed; a path it names that is not a `migrate` source simply has no matching `data.split_review` entry to affect (unlike `payload.split_sections`, which refuses an unknown path rather than ignoring it: a request for review that lands nowhere costs nothing, an accounting that lands nowhere would look validated). This operation is a pure function of `payload.sources`, `payload.answers`, `payload.split_requested`, `payload.split_sections`, and the current bytes of the sources it reads — nothing is stored between calls — so calling it again with the same inputs and unchanged sources always reproduces the same plan, `data.mapping`, `data.references`, `data.plan.duplicates`, and `data.split_review`, section ranges and `source_identity` included (#131's idempotency without resumability). Nothing carries an accepted accounting from one call to the next: each call re-derives and re-validates whatever this call was given.
...
7. **Derive the migration plan and run the one compact batched question round (#144).** Call `migration-plan` with `payload.sources` set to step 6's `data.sources`, unmodified. Done when a response is read with `data.plan` and `data.questions`; not done while any entry's disposition is assumed rather than read from `data.plan.entries`.
   ...
   - A determined plan's entry-level vocabulary (`migrate`/`skip`/`residue`/`blocked_pending_decision`, an intent) is deliberately distinct from `report`'s source-level vocabulary in step 16 (`migrated`/`skipped`/`ambiguous`/`residue`, an outcome): `data.mapping` already carries each `migrate` entry's extracted provenance and link-rewritten body (#145); step 8 partitions that same plan into the shards fresh-context workers actually convert, step 9 assembles what those workers actually returned, step 10 validates what was assembled, step 11 publishes what step 10 declared publishable, and step 16's payload is always built from what that work actually did, never copied straight from this step's plan.
8. **Partition the executable plan and dispatch one fresh-context worker per shard, in parallel (#146).** Once step 7's plan is executable, call `partition` with `payload.plan`, `payload.mapping`, and `payload.references` set to step 7's own `data.plan`, `data.mapping`, and `data.references`, unmodified, plus the bundle name and, when already known, `project_mode`. Done when `data.shards` is read and every shard's own `brief` is known; not done while a shard is dispatched from anything other than `data.shards[].brief` itself.
...
10. **Validate the staged bundle before publication (#148).** Once step 9's `assemble` call has run, call `migration-validate` with `payload.selected` set to step 6's own `data.sources` paths, `payload.plan` set to step 7's own `data.plan`, unmodified, and `payload.semantic_review` — honestly, not yet claiming a review that has not happened, since that comes only at step 15. Done when `data.status` and `data.publishable` are both read; not done while a structural finding, a `SOURCE_DISPOSITION_MISSING` finding, or a `data.publishable: false` is treated as anything other than blocking the next step.
    ...
    - `data.publishable: true` with no blocking finding is this procedure's own signal that step 11's publication may proceed; it never proceeds while `data.publishable: false`.
11. **Publish the validated staged bundle (#149).** Once step 10 reports `data.publishable: true`, call `publish` with `payload.staged` set to step 9's own `data.staged`, unmodified, and a write-eligible `payload.task_kind`. Done when `data.status`, `data.published`, and `data.failed` are all read; not done while `data.publishable: false` from step 10 is worked around, and not done while a concept in `data.failed` is treated as if it had been created.
    - `data.status: "complete"` — every staged concept's own delegation receipt was `"clean"`; the bundle now carries every concept step 9 staged.
    - `data.status: "partial"` — `data.failed` names each concept that did not publish and its own delegation `status`; a `CONCEPT_ALREADY_EXISTS` finding on one names a target collision with something already in the bundle (a decision for the user, same as step 9's own `CONCEPT_TARGET_COLLISION`, never resolved by retrying); any other finding names a write-gate refusal to correct before retrying just that concept. This is the operation this whole procedure exists to reach honestly: report exactly which concepts are real and which are not, never a blanket "migration complete" while `data.failed` is non-empty.
    - `result: "blocked"`, `data.code: "PUBLISH_PRECHECK_FAILED"` — the target bundle itself is not currently viable to write into (for example, the selected manifest bundle record no longer declares `okf_version: "0.2"`); nothing was attempted. Repair the manifest (step 4) and retry.
