...
Once collision-free, `assemble` writes one Markdown file per assembled concept — `type` frontmatter, plus `sources` only when that concept's own approved mapping declared it, plus `status: "draft"` for every type except `Glossary`, which #195 makes stable by default and which therefore carries no `status` at all — to `<Git root>/.okf-staging/<bundle>/<concept>.md`, a plain filesystem staging area beside the bundle, never inside it and never through the write gate: this is pre-publication scratch space, not itself required to be a conforming OKF bundle yet, and #148's later validation and a later publication step are what promote it into one. `data.staged` names one `{ "path", "concept", "type", "shard", "file", "sources" }` per file actually written, where `sources` (#174) is that concept's own observation binding: `[{ "path", "sha256" }]` for the accepted source file it was migrated from, hashed as it stands now, so `publish` can cite the real source instead of filler. One source hashed once binds every concept split out of it; a source that cannot be read yields an empty list, never an invented identity, `data.staging_dir` names the staging root (`Git`-root-relative), and `data.status` is `"complete"` (`data.publishable: true`) only when no shard reported a blocker.
...
Nothing here binds a staged **file** to the Concept ID it is staged under either. `publish` writes the bytes of `payload.staged[].file` to `<concept>.md`, and every check above reads that file's own frontmatter without ever asking whether it is the file for that concept: two staged entries of the same type, from the same source, with their `file` paths exchanged, publish each other's bodies and every dimension of this comparison still passes.

The response is `result: "ok"` with `data.status` (`"complete"` only when nothing found above blocks, `"partial"` otherwise), `data.publishable` (the same test, named plainly), `data.missing_disposition` (every `selected` path this call could not find an accepted source row for), `data.concepts_checked` (every staged path this call actually read), `data.conformance` (`{ "findings": <count> }` — a count of what this run found, not a receipt and not an approval: `publish` never reads it and runs the whole comparison again itself), and `data.semantic_fidelity`. This operation never recomputes `report`'s own summary statistics or renders any prose; a caller who wants the post-setup analytics still calls `report` itself, separately, once this gate is satisfied.
