...
- `migrate` — `data.plan.entries[].concept` (the bundle-relative concept path, the source path with its extension stripped) and `.type` are both set. `reason: "type_preserved"` when the source's own frontmatter already named a non-empty `type` (read through the same frontmatter/YAML reader `discover` and the write gate both use, and preserved verbatim — never re-guessed); `reason: "type_approved"` when `payload.answers` supplied the type instead.
...
`migration-plan` (#144) shares `discover`'s exact admission: no REACH/TRUST/ACCESS, no evidence, the ordinary activation gate rather than a bypass, because checking a candidate target path for a collision needs the bundle root to exist. It runs no write gate either — it never writes, it only reads each markdown source's own frontmatter and probes the bundle for an existing file at the candidate target path.
...
8. **Derive the migration plan and run the one compact batched question round (#144).** Call `migration-plan` with `payload.sources` set to step 7's `data.sources`, unmodified. Done when a response is read with `data.plan` and `data.questions`; not done while any entry's disposition is assumed rather than read from `data.plan.entries`.
   ...
   - A determined plan's entry-level vocabulary (`migrate`/`skip`/`residue`/`blocked_pending_decision`, an intent) is deliberately distinct from `report`'s source-level vocabulary in step 13 (`migrated`/`skipped`/`ambiguous`/`residue`, an outcome): converting each `migrate` entry into an actual concept is the migration worker's job (#145 onward), not this step's, and step 13's payload is always built from what that work actually did, never copied straight from this step's plan.
