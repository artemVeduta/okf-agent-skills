...
1. Resolves every staged file against `<Git root>/.okf-staging/<bundle>`, refuses a symlinked staging root, ancestor, or file, and reads each candidate once as canonical checked bytes. It proves exact one-to-one plan/mapping source and concept coverage, exact candidate path sets and counts, exact canonical review source and section coverage, every current source identity, actual concept frontmatter/body fields, actual authored provenance, actual group placement, generated index title/purpose/ordered children, and exact ordinary/heading-anchor/whole-source route target multisets with fragments retained. A value it cannot prove blocks; it never guesses.
