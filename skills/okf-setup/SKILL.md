...
4. **Repair `.okf-workspace.json` through `repair`, when approved.** Read `inspect`'s report for this file first:
...
11. **Publish the validated staged bundle (#149).** Once step 10 reports `data.publishable: true`, call `publish` with `payload.staged` set to step 9's own `data.staged`, unmodified, and a write-eligible `payload.task_kind`. Done when `data.status`, `data.published`, and `data.failed` are all read; not done while `data.publishable: false` from step 10 is worked around, and not done while a concept in `data.failed` is treated as if it had been created.
    ...
    - `result: "blocked"`, `data.code: "PUBLISH_PRECHECK_FAILED"` — the target bundle itself is not currently viable to write into (for example, `index.md` no longer declares `okf_version: "0.2"`); nothing was attempted. Repair the bundle root (step 5) and retry.
