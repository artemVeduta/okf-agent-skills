...
```json
{
  ...
  "payload": {
    ...
    "mapping": [{ "path": "docs/decisions/use-postgres.md", "concept": "decisions/use-postgres", "type": "Decision", "sources": null, "body": "# Use PostgreSQL\n" }],
...
1. Resolves every `file` against `<Git root>/.okf-staging/<bundle>` and reads the complete current staged Markdown set. Before write 1 it proves exact candidate count and paths, unsplit outputs required by the plan, and every split output's Concept ID, path, type, title, heading outline, source-section assignments and order, group and index row, provenance, links, and anchors. It compares real frontmatter, body, and assembly metadata with the accepted proposal. A value it cannot prove blocks; it never guesses.
...
Immediately before write 1, after the delegated target-bundle read, `publish` reruns all three Task 6 checks against current bytes: every source identity, complete candidate-set equality, and canonical semantic-review freshness. Review freshness requires exact current candidate identities, exact accepted sections, output mapping, and proposal, and only `preserved` section verdicts. `human_assessed` remains separate and cannot replace the agent review. Any precheck failure is `result: "blocked"`, `data.code: "PUBLISH_PRECHECK_FAILED"`, with `PUBLISH_CANDIDATE_SET_MISMATCH`, `PUBLISH_CANDIDATE_CHANGED`, `PUBLISH_CANDIDATE_SCAN_INCOMPLETE`, `PUBLISH_CANDIDATE_READ_FAILED`, `PUBLISH_SOURCE_CHANGED`, or `PUBLISH_SEMANTIC_REVIEW_STALE`. A candidate mismatch requires one new complete proposal. A precheck failure writes zero bundle files.
...
- `publish` stores no call state. A later call must again supply the complete accepted plan, mapping, split review, staged set, and canonical semantic review. A concept already created by an earlier call is an ordinary `CONCEPT_ALREADY_EXISTS` refusal. Do not narrow `payload.staged`; candidate-set equality requires the complete accepted set.
