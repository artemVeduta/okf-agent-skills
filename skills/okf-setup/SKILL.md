...
- `data.plan`, `data.mapping`, `data.references` — the accepted proposal projected into exactly the shapes `partition` already consumes, with the accepted target paths and each body's links rewritten against them. Each `data.mapping` row additionally carries the accepted `content_scope` and `source_anchors` of its own output, so a split's parts reach two different workers with two different content boundaries rather than the identical whole body. `data.plan.executable` is `true` only when the proposal is acceptable, and a rejected proposal carries an empty `mapping`, empty `navigation`, and `executable: false`, so nothing downstream can transform or publish from it.
