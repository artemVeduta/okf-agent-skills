...
const MERGE: Scenario[] = [
  ...
  S('M-R-06', 'crash with in-flight recorded', merge(), K('aglkmnx'),
    all(phase('unknown-interrupted'),
      check('settlement is indeterminate', (w) => frameOf(w).classification.settlement === 'indeterminate'),
...
const SUPERSEDE: Scenario[] = [
  ...
  S('P-F-03', 'backlink recorded but archive relocation failed', supersede({ archiveRelocate: true, supersedeEdge: 'superseded_by-field' }), K('aglkmnf'),
    all(phase('failed-dirty'),
  ...
  S('P-R-03', 'relocated predecessor means a reverse move', supersede({ archiveRelocate: true }), K('aglkmnnfBN'),
    all(phase('reverted-clean'),
      check('the inverse carries its own lineage and its own GATE', () => {
