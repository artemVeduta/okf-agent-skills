 ...
 * an in-memory multi-bundle corpus, a crude planner that turns a requested
 * operation into a manifest of atomic effects, and fault switches (write fails
 * at effect k, a concurrent edit arrives between preview and apply, restore
 * fails, the process dies between the write and the journal append).
 *
...
// ---------------------------------------------------------------------------
// Fault switches, all consumed by driver.ts.
// ---------------------------------------------------------------------------

export interface Faults {
  /** Effect ordinal whose write fails; `null` for none. */
  failAt: number | null;
  /** Effect ordinal at which a concurrent change is detected mid-apply. */
  concurrentAt: number | null;
  /** Effect ordinal whose produced bytes differ from the sealed post-image. */
  deviateAt: number | null;
  /** Inverse-step ordinal whose restore is not byte-identical. */
  restoreFailsAt: number | null;
  /** Effect ordinal after whose write the process dies before the journal append. */
  dieAfterWriteAt: number | null;
}

export function noFaults(): Faults {
  return { failAt: null, concurrentAt: null, deviateAt: null, restoreFailsAt: null, dieAfterWriteAt: null };
}
