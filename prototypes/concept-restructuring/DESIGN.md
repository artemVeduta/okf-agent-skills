...
| T19 | `crash` | `manifest-durable` \| `mutating` \| `verifying` \| `rolling-back` | `unknown-interrupted` | Process death with something in flight. A rollback segment records `MANIFEST_DURABLE` after its inverse manifest is admitted and before any inverse write. A crash during rollback therefore reaches `unknown-interrupted` after that durable record, while `derivePhase` still checks `INTERRUPTED` before the rollback branch because the corpus may be half-restored under two live identities | Appends nothing. The journal ends with a dangling `INTENT` or a complete apply with no `SETTLED` |
