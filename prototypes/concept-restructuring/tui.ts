 ...
 * state. It is collapsed to fit one screen — nothing scrolls, everything
 * truncates, and every count that was dropped is stated.
 */
...
function render(): void {
  ...
  if (rows.length > 7) out(`  ${D}${more(7, rows.length).trim()}${R}`);
  ...
  for (const s of m.steps.slice(0, 8)) {
  ...
  out(`  ${D}${m.steps.length} step(s)${more(8, m.steps.length)}; ${m.rollbackSteps.length} approved inverse step(s)${R}`);
  ...
  out(
    `  ${D}${more(4, f.trust.length).trim()}  review dependencies: ${f.reviewDependencies
