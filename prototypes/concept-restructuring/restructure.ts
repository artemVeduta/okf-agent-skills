...
function recoveryFailures(e: RecoveryEvidence, approved: ApprovedPlan | null): readonly string[] {
  const out = [...failedConjuncts(e)];
  if (approved && approved.recoveryEvidenceHash !== null && e.evidenceHash !== approved.recoveryEvidenceHash) {
    out.push(`evidenceHash=${e.evidenceHash} does not match recoveryEvidenceHash=${approved.recoveryEvidenceHash}`);
  }
  return out;
}
...
export function classify(p: Phase): Classification {
  switch (p) {
    ...
    case 'verifying':
    case 'rolling-back':
      return { settlement: 'none', cleanliness: 'clean', terminal: false };
    case 'refused':
    case 'gate-blocked':
    case 'expired':
      return { settlement: 'none', cleanliness: 'clean', terminal: true };
    case 'failed-clean':
      return { settlement: 'failed', cleanliness: 'clean', terminal: true };
    case 'applied-clean':
    case 'applied-with-known-breakage':
      return { settlement: 'applied', cleanliness: 'clean', terminal: true };
    case 'reverted-clean':
...
function residueOf(j: Journal): readonly Residue[] {
  const currentManifest = findManifest(currentSegment(j));
  if (!currentManifest || currentManifest.revertOf === null) return [];
...
function ambiguitiesOf(j: Journal, world: readonly Observed[], phase: Phase): readonly AmbiguityFinding[] {
  ...
  for (const r of seg) if (r.r === 'ACKNOWLEDGED') acked.add(r.kind);
...
export function admissionRefusal(
  ...
  for (const s of steps) {
    if (s.kind !== 'CREATE_OUTPUT') continue;
    if (s.outputDraft === null) {
      return { code: 'OUTPUT_NOT_EXPLICIT_DRAFT', detail: [`output ${ks(s.target)} has no explicit draft facts`] };
    }
 ...
 *
 * `observe` is legal everywhere. `acknowledge` is also routed everywhere, but
 * it requires a current ambiguity; neither action advances the operation.
 */
...
export function reduce(j: Journal, world: readonly Observed[], a: Action): Step {
  ...
  switch (a.kind) {
    ...
    case 'gate': {
      ...
      const failed = recoveryFailures(a.evidence, findApproved(seg));
    ...
    case 'lock': {
      ...
      if (has(seg, 'LOCKED')) {
        return result(j, world, 'REFUSE', 'ALREADY_LOCKED', ['the current segment already has a LOCKED record']);
      }
    ...
    // ---- T19 ------------------------------------------------------------
    case 'crash': {
      // A rollback segment records MANIFEST_DURABLE before its first inverse
      // step. Being IN FLIGHT remains the test here so a crash during rollback
      // reaches `unknown-interrupted` while the corpus is half-restored.
    ...
    case 'beginRollback': {
      ...
      const landedTargets = new Set(
        stepObservations(j)
          .filter(([, o]) => o.state !== 'not-started')
          .map(([ordinal]) => manifest.steps.find((s) => s.ordinal === ordinal)?.target)
          .filter((key): key is ConceptKey => key !== undefined)
          .map(ks),
      );
      const missingInverse = [...landedTargets].filter(
        (key) => !manifest.rollbackSteps.some((s) => ks(s.target) === key),
      );
      if (missingInverse.length > 0) {
        return result(j, world, 'BLOCK', 'INVERSE_STEP_NOT_APPROVED', [
          `no approved inverse step for ${missingInverse.join(', ')}`,
        ]);
      }
    ...
    // ---- T29 ------------------------------------------------------------
    case 'acknowledge': {
      const frame = derive(j, world);
      if (!frame.ambiguities.some((finding) => finding.kind === a.ambiguity)) {
        return result(j, world, 'REFUSE', 'AMBIGUITY_NOT_PRESENT', [
          `ambiguity ${a.ambiguity} is not present in the current frame`,
        ]);
      }
