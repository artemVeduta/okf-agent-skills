...
| `okf-setup` | Inspects the three `/setup` config files and, once approved, bootstraps the bundle-root `index.md` via `init` and repairs `.okf-active`/`.okf-workspace.json`; for a monorepo, `plan` detects package boundaries and builds one sub-agent brief per package and `aggregate` reports their results into the shared workspace manifest; `discover` scans the active project and classifies each candidate source document as markdown, unsupported, other, or ambiguous; `migration-plan` turns that inventory into a fully-determined migration plan — migrate, skip, retain as residue, or blocked pending one compact batched round of user decisions; `report` turns migration signals into post-setup analytics for the agent to render. Direct invocation only. |
